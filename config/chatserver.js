...
// ERROR HANDLING
    ...
    user.socket.on('doesSearchChatroomExist', function(homeRoomQuery) {
      console.log('e.doesSearchChatroomExist');
      self.doesSearchChatroomExist(user, homeRoomQuery);
    });
  ...
  self.getUsersAndHeader = function(user, roomName) {
    ...
    ChatroomModel.findOne({name: roomName}, function( err, chatroom ) {
      if (!err) {
        ...
        user.socket.emit('chatroomHeader', {id: chatroom._id, name: roomName, roomImage: chatroom.roomImage, privacy: chatroom.privacy, owner: chatroom.owner, currentUser: user.username, chatlogLength: chatroom.chatlog.length, modelsLoadedSum: -1, userImage: user.userImage, homeRoom: user.homeRoom});
      } else {
        return console.log (err);
      }
    });
...
// ERROR HANDLING
    ...
    self.doesSearchChatroomExist = function(user, chatroomQuery) {
      console.log('chatroomQuery: ', chatroomQuery);
      ChatroomModel.findOne({ name: chatroomQuery }, function(err, chatroom) {
        if (!chatroom) {
          user.socket.emit('chatroomExists', true);
        } else {
          user.socket.emit('chatroomExists', false);
        }
      });
    };
